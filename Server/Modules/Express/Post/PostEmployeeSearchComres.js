 ...
 * @module Express/Post/PostEmployeeSearchComres
 *
...
async function action_p (req, client) {
    ...
    if (searchType == 'productID') {
        var searchRes  = await t_db.execOptDataQuery_p(SQL.ADVANCED.SEARCH_COMRES_BY_PRODUCT_ID, [searchTerm], true, client);
        if (searchRes.errorList.length > 0) {
            error(req, 'Failed to search for comres by productID.');
            t_misc.addError(result.errorList, 'DB_COMRES_SEARCH_BY_PRODUCT_ID_FAIL', searchRes.errorList[0]);
            return result;
        }
        ...
        var searchRes  = await t_db.execOptDataQuery_p(SQL.ADVANCED.SEARCH_COMRES_BY_PRODUCT_NAME, [searchTerm], true, client);
        if (searchRes.errorList.length > 0) {
            error(req, 'Failed to search for comres by product name.');
            t_misc.addError(result.errorList, 'DB_COMRES_SEARCH_BY_PRODUCT_NAME_FAIL', searchRes.errorList[0]);
            return result;
        }
        result.rows   = searchRes.rows;
        result.fields = searchRes.fields;
...
// EXPORT
// *********************************
module.exports = async function postEmployeeSearchComres_p (req, res, client) {
    printraw('');
    print(req, '(Start) POST /employee/search/comres');
    ...
    if (suiteResult_p.errorList.length > 0) {
        ...
        res.send({ error: 'POST_EMPLOYEE_SEARCH_COMRES_FAILED' });
        warn(req, '(End) Employee comres search failed. Response sent.');
    }
    else {
        res.send({ 
            error  : null,
        ...
        success(req, '(End) Employee comres search success. Response sent.');
    }
}
