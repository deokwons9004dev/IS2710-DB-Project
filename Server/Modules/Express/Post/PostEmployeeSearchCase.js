 ...
 * @module Express/Post/PostEmployeeSearchCase
 *
...
async function action_p (req, client) {
    ...
    if (searchType == 'status') {
        var searchData = [searchTerm];
        var searchRes  = await t_db.execOptDataQuery_p(SQL.CASES.SEARCH_BY_STATUS, searchData, false, client);
        if (searchRes.errorList.length > 0) {
            error(req, 'Failed to search for cases by status.');
            t_misc.addError(result.errorList, 'DB_CASES_STATUS_SEARCH_FAIL', searchRes.errorList[0]);
            return result;
        }
        result.rows   = searchRes.rows;
        result.fields = searchRes.fields;
    }
    else if (searchType == 'timeframe') {
        var searchData = [searchTerm];
        var searchRes  = await t_db.execOptDataQuery_p(SQL.CASES.SEARCH_BY_OPENTIME, searchData, false, client);
        if (searchRes.errorList.length > 0) {
            error(req, 'Failed to search for cases by timeframe.');
            t_misc.addError(result.errorList, 'DB_CASES_OPENTIME_SEARCH_FAIL', searchRes.errorList[0]);
            return result;
        }
        result.rows   = searchRes.rows;
        result.fields = searchRes.fields;
...
// EXPORT
// *********************************
module.exports = async function postEmployeeSearchCase_p (req, res, client) {
    printraw('');
    print(req, '(Start) POST /employee/search/case');
    ...
    if (suiteResult_p.errorList.length > 0) {
        ...
        res.send({ error: 'POST_EMPLOYEE_SEARCH_CASE_FAILED' });
        warn(req, '(End) Employee case search failed. Response sent.');
    }
    else {
        res.send({ 
            error  : null,
        ...
        success(req, '(End) Employee case search success. Response sent.');
    }
}
