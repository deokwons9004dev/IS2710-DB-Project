 ...
 * @module Express/Post/PostDBQuery
 *
...
// EXPORT
// *********************************
module.exports = async function postDBQuery_p (req, res, client) {
    printraw('');
    print(req, '(Start) POST /db/query');

    log(req.body);

    if (!req || !req.body || !req.body.querySQL) {
        error('Request body or querySQL not given.');
        res.send({ error: 'REQUEST_BODY_QUERYSQL_NOT_FOUND' });
        return;
    }

    var querySQL                 = req.body.querySQL;
    var keepDuplicateColumnNames = (req.body.keepDuplicateColumnNames == 'true');

    var queryResult = await t_db.execOptQuery_p(querySQL, keepDuplicateColumnNames, client);
    if (queryResult.errorList.length > 0) {
        error('Failed to complete query.');
        error(queryResult.errorList);
        res.send({ error: 'DB_QUERY_EXEC_FAIL' });
        return;
    }
    ...
    success(req, '(End) Query data sent.');
}
